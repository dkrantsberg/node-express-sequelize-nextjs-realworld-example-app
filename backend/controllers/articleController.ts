...
// Preload the article referenced by a ':article' route param (by slug).
export function preloadArticle(
  req: Request,
  res: Response,
  next: NextFunction,
  slug: string
...
// Preload the comment referenced by a ':comment' route param (by id).
export function preloadComment(
  req: Request,
  res: Response,
  next: NextFunction,
  id: string
...
// List articles (with optional author/favorited/tag filters).
...
// The feed of articles authored by users the logged-in user follows.
...
// Get a single article (preloaded via ':article').
